...
namespace AbpPoc.Ipbs
{
    ...
    public abstract class IpbsAppServiceBase : AbpPocAppService
    {
        ...
        {
            var totalCount = await _ipbRepository.GetCountAsync(input.FilterText, input.ipbIndex, input.figureName, input.figureNumber, input.toNumber, input.indentureLevel, input.sourceId, input.relatedId, input.sourcePart, input.relatedPart);
            var items = await _ipbRepository.GetListWithNavigationPropertiesAsync(input.FilterText, input.ipbIndex, input.figureName, input.figureNumber, input.toNumber, input.indentureLevel, input.sourceId, input.relatedId, input.sourcePart, input.relatedPart, input.Sorting, input.MaxResultCount, input.SkipCount);
        ...
        public virtual async Task<IpbDto> CreateAsync(IpbCreateDto input)
        {
            ...
            input.sourcePart, input.relatedPart, input.ipbIndex, input.figureName, input.figureNumber, input.sourceId, input.relatedId, input.toNumber, input.indentureLevel
            );
        ...
        public virtual async Task<IpbDto> UpdateAsync(Guid id, IpbUpdateDto input)
        {
            ...
            id,
            input.sourcePart, input.relatedPart, input.ipbIndex, input.figureName, input.figureNumber, input.sourceId, input.relatedId, input.toNumber, input.indentureLevel, input.ConcurrencyStamp
            );
